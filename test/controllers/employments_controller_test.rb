...
class EmploymentsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create employment" do
    assert_difference('Employment.count') do
      post employments_url, params: { employment: { employer: @employment.employer, revenues: @employment.revenues, since: @employment.since, status: @employment.status, title: @employment.title, user_id: @employment.user_id } }
    end
  ...
  test "should update employment" do
    patch employment_url(@employment), params: { employment: { employer: @employment.employer, revenues: @employment.revenues, since: @employment.since, status: @employment.status, title: @employment.title, user_id: @employment.user_id } }
